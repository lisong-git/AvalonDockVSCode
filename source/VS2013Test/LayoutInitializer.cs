...
namespace AvalonDock.VS2013Test {
	class LayoutInitializer : ILayoutUpdateStrategy {
		public bool BeforeInsertAnchorable(LayoutRoot layout, LayoutAnchorable anchorableToShow, ILayoutContainer destinationContainer) {
			...
			Debug.WriteLine($"{anchorableToShow.Content?.GetType().Name}", "BeforeInsertAnchorable 1");
			...
			if (anchorableToShow.Content is PropertiesViewModel) {
				...
				if (propertiesPane != null) {
					...
					p.Children.Add(propertiesPane);
				}
				return true;
			}

			//if (anchorableToShow.Content is OutputViewModel) {
			//	var outputPane = layout.Descendents().OfType<LayoutAnchorableGroup>().FirstOrDefault(d => d.Name == "OutputPane");
